...
export default function CRMView({ searchQuery }) {
  ...
  if (loading) return <div className="text-brand-gold animate-pulse text-sm uppercase py-12">Syncing client network...</div>;

  return (
    <div className="space-y-10">
      <header className="flex items-center justify-between border-b border-brand-border pb-6">
        <div>
          <h2 className="font-serif text-2xl tracking-wide text-white">VIP Network (CRM)</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredCustomers.length === 0 ? (
          <div className="col-span-full text-center py-20 text-gray-500 font-mono text-sm border border-brand-border border-dashed rounded-xl">
          ...
          return (
            <div
              key={c._id}
              className="group relative rounded-[1.25rem] border border-brand-border bg-brand-gray transition-all hover:border-brand-gold/40 hover:shadow-2xl overflow-hidden"
            >
              {/* Card Header */}
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => startEdit(c)} className="p-1.5 text-gray-500 hover:text-brand-gold bg-black/40 rounded-lg"><Edit3 size={13} /></button>
                    <button onClick={() => deleteCustomer(c._id)} className="p-1.5 text-gray-500 hover:text-red-400 bg-black/40 rounded-lg"><Trash2 size={13} /></button>
                  </div>
                </div>

                <h4 className="font-serif text-lg text-white mb-1">{c.name}</h4>
                ...
                <div className="mt-4 pt-4 border-t border-brand-border/30 flex justify-between items-end">
                  <div>
                    <p className="text-[0.6rem] uppercase text-gray-500 tracking-widest">Total Spend</p>
                    <p className={`font-serif font-bold text-lg ${totalSpend > 0 ? 'text-brand-gold' : 'text-gray-500'}`}>
                      ${totalSpend.toLocaleString()}
                    </p>
                    <p className="text-[0.6rem] text-gray-600">{custSales.length} purchase{custSales.length !== 1 ? 's' : ''}</p>
                  </div>
                  <button
                    onClick={() => setExpandedId(isExpanded ? null : c._id)}
                    className="flex items-center gap-1.5 text-[0.65rem] uppercase tracking-widest font-bold text-brand-gold hover:text-white transition-colors"
                  >
              ...
              {isExpanded && (
                <div className="border-t border-brand-border bg-black/40 px-6 py-4 space-y-3">
                  <p className="text-[0.6rem] uppercase tracking-widest text-gray-500 font-bold mb-3">Purchase History</p>
                  {custSales.length === 0 ? (
                    <p className="text-xs text-gray-600 font-mono">No purchases recorded yet.</p>
                  ) : custSales.map(s => (
                    <div key={s._id} className="flex items-center justify-between py-2 border-b border-brand-border/30 last:border-0">
                      <div className="flex items-start gap-3">
                        <div className="mt-0.5 p-1 rounded-md bg-brand-gold/10 text-brand-gold">
                          <ShoppingBag size={11} />
                        </div>
                        <div>
                          <p className="text-xs text-white font-medium">
                            {s.product?.name || 'Deleted Product'}
                            <span className="text-gray-500 font-mono ml-1.5">×{s.quantitySold}</span>
                          </p>
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className={`text-[0.55rem] font-bold uppercase px-1.5 py-0.5 rounded-full ${
                              String(s.status).toLowerCase().includes('refund') ? 'bg-red-400/20 text-red-400' :
                              String(s.status).toLowerCase().includes('unpaid') ? 'bg-amber-400/20 text-amber-400' :
                              'bg-green-400/20 text-green-400'
                            }`}>{s.status || 'Paid'}</span>
                          </div>
                        </div>
                      </div>
                      <span className={`font-serif font-bold text-sm shrink-0 ${String(s.status).toLowerCase().includes('refund') ? 'text-red-400 line-through' : 'text-brand-gold'}`}>
                  ...
                  {custSales.length > 0 && (
                    <div className="flex justify-between pt-2 border-t border-brand-border/50">
                      <span className="text-[0.6rem] uppercase tracking-widest text-gray-500">Net Spend</span>
                      <span className="font-serif font-bold text-brand-gold">${totalSpend.toLocaleString()}</span>
                    </div>
                  )}
                </div>
              )}
