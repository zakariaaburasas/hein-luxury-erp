  ...
  if (!data || data.length === 0) {
    return (
      <div className="rounded-[1.25rem] bg-brand-gray border border-brand-border p-6 shadow-lg h-full flex flex-col justify-center items-center">
        <p className="text-gray-500 text-sm font-mono text-center">No revenue data yet. Log your first sale to see growth.</p>
      </div>
    );
  }

  return (
    <div className="rounded-[1.25rem] bg-brand-gray border border-brand-border p-6 shadow-lg h-full flex flex-col relative overflow-hidden">
      {isRestricted && (
        <div className="absolute inset-0 z-10 bg-brand-gray/40 backdrop-blur-md flex items-center justify-center p-8 text-center">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">Monthly performance</p>
        </div>
        {!isRestricted && <span className="text-xs bg-brand-gold/10 border border-brand-gold/30 text-brand-gold px-3 py-1 rounded-full font-mono">LIVE</span>}
      </div>
      <div className={`flex-1 min-h-[200px] ${isRestricted ? 'blur-sm grayscale' : ''}`}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
            ...
            <YAxis hide={isRestricted} axisLine={false} tickLine={false} tick={{ fill: '#6b7280', fontSize: 10 }} tickFormatter={v => `$${v}`} />
